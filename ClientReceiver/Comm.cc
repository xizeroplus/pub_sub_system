...
//
//
// $Id: Comm.cc,v 1.2 2006/08/15 18:37:38 carzanig Exp $
//
...
const char * BadSender::what() const throw () { 
    return "invalid Sender";
}

const char * BadSenderSchema::what() const throw () { 
    return "invalid Sender: unknown schema";
}

const char * UnknownHost::what() const throw () { 
    return "invalid Sender: unknown host";
}

const char * CommException::what() const throw () { 
...
const char * CommInterrupted::what() const throw () { 
...
const char * SocketInterrupted::what() const throw () { 
    static const char * _descr[] = { 
	"socket interruption", 
...
static const char * TCPSenderSchema = "tcp:";
static const char * OldTCPSenderSchema = "senp:";
static const char * UDPSenderSchema = "udp:";
static const char * OldUDPSenderSchema = "udp+senp:";
		     ...
		     string::const_iterator end)
{
    ...
    if (curs != end && *curs == '/') {
	++curs;
	if (curs == end || *curs++ != '/') throw BadSender();
    }
    //
...
Sender * Sender::create_sender(const string & uri) {
    ...
    } else if ((i = match_prefix(uri, OldTCPSenderSchema)) != uri.end()) {
	parse_host_port(host, port, i, uri.end());
	return new TCPSender(host,port);
    } else if ((i = match_prefix(uri, OldUDPSenderSchema)) != uri.end()) {
	parse_host_port(host, port, i, uri.end());
	return new UDPSender(host,port);
    } else {
...
void TCPSender::send(const char * buf, size_t s) throw (CommException) {
    ...
    if (connect(sock_fd, (LPSOCKADDR)(& addr), sizeof(struct sockaddr)) 
        == SOCKET_ERROR) {
	    closesocket(sock_fd);
...
void UDPSender::send(const char * buf, size_t s) throw (CommException) {
    ...
    if(sendto(sock_fd, buf, s, 0, (LPSOCKADDR)&addr, sizeof(struct sockaddr))
       == SOCKET_ERROR) 
	throw SocketError(SocketError::SendTo, 0);
}
#else
...
TCPReceiver::TCPReceiver(in_port_t p, const char * hostname, 
    ...
    // 
    getsockname(sock_fd, (sockaddr *)&sockInfo, (socklen_t *)&sockInfoLen);
...
#ifdef USING_WIN32
UDPReceiver::UDPReceiver(in_port_t p, const char * hostname, bool reuse)
    : port(p), sock_fd(INVALID_SOCKET) {
    sock_fd = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (sock_fd == INVALID_SOCKET)
	throw SocketError(SocketError::Create, errno);
